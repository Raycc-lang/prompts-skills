...
|---|---|---|---|---|
| 1 | Name-only (只记名称) | Uses the term fluently; cannot explain or apply anything behind it | Only a language-layer symbol stored; no concept, no model, no reference target | Run the full loop from step 0; treat the term as unlearned. Name-dropping is sometimes a legitimate social tactic — ask about intent |
